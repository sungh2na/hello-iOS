...
class PlayerViewController: UIViewController {
    ...
    var timeObserver: Any?      // 시간을 관찰함. 플레이어에서 곡 재생할때 재생되는 것의 시간을 관찰
    ...
    override func viewDidLoad() {       // 메모리에 로드 됐을 때 호출
        ...
        // 일정 간격으로 시간 관찰하는 것 더함
//        CMTime(seconds: 1, preferredTimescale: 10) // AVFoundation 밑에 있음, 단위 초를 쪼개고 싶은 양으로 표현, 1초를 10개로 나눔 -> 0.1초
        timeObserver = simplePlayer.addPeriodicTimeObserver(forInterval: CMTime(seconds: 1, preferredTimescale: 10), queue: DispatchQueue.main, using: { time
            in self.updateTime(time: time) })    // 해당하는 곡의 현재시간을 받아서 업데이트
    }
